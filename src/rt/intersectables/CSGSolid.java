...
public abstract class CSGSolid implements Intersectable {
	...
	public static Point2f solveQuadratic(float a, float b, float c) {
		...
		if(disc>0){
			result.x=(float) (-b+Math.sqrt(disc))/(2*a);
			result.y=(float) (-b-Math.sqrt(disc))/(2*a);
		}
	...
	public BoundingBox getBoundingBox(){
		return new BoundingBox(Float.NEGATIVE_INFINITY,Float.NEGATIVE_INFINITY,Float.NEGATIVE_INFINITY,
